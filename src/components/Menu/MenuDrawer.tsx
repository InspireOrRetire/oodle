...
export default function MenuDrawer({ isOpen, onClose }: Props) {
  ...
  const balance = 0           // wallet balance — not in schema yet
  const tokenBalance = 120    // token balance  — not in schema yet
      ...
      <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pt-4 px-4">
          ...
          <div className="space-y-1 mb-1">
            {[
              { icon: Users, label: 'Discover Friends' },
              ...(isCreator ? [{ icon: Zap, label: 'Creator Center' }] : []),
              { icon: FileText, label: 'Drafts', badge: '1' },
              { icon: MessageSquare, label: 'My Comments' },
              { icon: History, label: 'History' },
            ].map(item => (
              <button key={item.label} className="w-full flex items-center justify-between py-3 px-3 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3">
                  <item.icon className="w-5 h-5 text-gray-600" />
                  <span className="text-[15px] text-gray-900">{item.label}</span>
                </div>
                {'badge' in item && item.badge && (
                  <div className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{item.badge}</div>
                )}
              </button>
            ))}
          </div>
          ...
          <button className="w-full flex items-center gap-3 py-3 px-3 rounded-lg hover:bg-gray-50">
