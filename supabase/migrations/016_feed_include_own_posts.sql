-- ============================================================
-- 016 — Include user's own posts in home feed
-- ============================================================
-- The previous get_feed_data excluded the calling user's own
-- posts from both sections:
--   • followed  → only posts from creators you follow
--   • discovery → explicit `AND posts.creator_id != p_user_id`
-- This meant your own posts never appeared in your home feed.
-- Fix: treat your own posts the same as followed-creator posts.
-- ============================================================
...
BEGIN
  ...
  -- Followed posts + own posts: last 48h, newest first, max 60
  SELECT JSON_AGG(row_to_json(p)) INTO v_followed_posts FROM (
    ...
    WHERE (
      posts.creator_id = ANY(v_followed_ids)   -- creators you follow
      OR posts.creator_id = p_user_id           -- your own posts
    )
  ...
  -- Discovery posts: non-followed, excluding own, last 72h, max 100
