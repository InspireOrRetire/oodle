-- ============================================================
-- 008 — Location fields on posts + updated feed RPC
-- ============================================================

-- Add location columns to posts (idempotent)
ALTER TABLE posts
  ADD COLUMN IF NOT EXISTS location_address TEXT,
  ADD COLUMN IF NOT EXISTS location_lat     FLOAT8,
  ADD COLUMN IF NOT EXISTS location_lng     FLOAT8;

-- ── Rebuild get_feed_data to include location_address ─────────────────────────
-- Both the followed and discovery arms now SELECT location_address so that
-- the client can render the 📍 chip on feed cards without a second round-trip.
